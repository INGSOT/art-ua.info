...
const FORM_FIELDS = [
  { id: "name", label: "Як до вас звертатись", placeholder: "Ваше ім'я та прізвище", type: "text" as const },
  { id: "email", label: "Електронна пошта", placeholder: "Вкажіть адресу електронної пошти", type: "email" as const },
  { id: "phone", label: "Телефон", placeholder: "Вкажіть номер телефону", type: "tel" as const },
  { id: "message", label: "Повідомлення", placeholder: "Ваше повідомлення", type: "textarea" as const, rows: 5 },
];
  ...
  if (notFound || !service) {
    return (
      <>
        <Header isHomePage={false} />
        <div className="w-full min-h-[420px] flex items-center justify-center bg-[#414141]">
          <p className="font-wix text-white text-lg md:text-2xl">Послугу не знайдено</p>
        </div>
        <Footer />
      </>
    );
    ...
    <>
      ...
      <div className="flex flex-col lg:flex-row w-full">
        ...
        <div className="w-full lg:w-1/2 flex flex-col">
          ...
          <OrderForm slug={service.slug} options={service.options} />
        </div>
      </div>
      <Footer />
    </>
...
function OrderForm({ slug, options }: { slug: string; options: string[] }) {
  ...
  const validateForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotification({ type: "error", text: getApiErrorMessage(error, "Не вдалося надіслати запит") });
    } finally {
        ...
        <div className="flex flex-col gap-6">
          ...
          >
            <span className="flex items-center justify-center flex-1 px-6 font-bold text-[#343434]">
              {isSubmitting ? "Надсилаємо..." : "Відправити запит"}
            </span>

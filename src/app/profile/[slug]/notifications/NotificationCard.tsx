...
import { notificationsTexts } from "../../../../data/profileData";
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col gap-3 text-white">
        <span className="text-[#FECC39] text-xs font-bold uppercase tracking-wide">Замовлення послуги</span>
        ...
        {(customerName || customerEmail || customerPhone || customerMessage) && (
          <div className="font-wix flex flex-col gap-1 text-sm text-white/80">
            {customerName && <span>Замовник: {customerName}</span>}
            {customerEmail && <span>Email: {customerEmail}</span>}
            {customerPhone && <span>Телефон: {customerPhone}</span>}
            {options.length > 0 && <span>Опції: {options.join(", ")}</span>}
            {customerMessage && <span>Повідомлення: {customerMessage}</span>}
          </div>
        )}

        {serviceSlug && (
          <Link href={`/services/${serviceSlug}`} className="w-fit text-[#FECC39] font-bold text-sm hover:text-white">
            {notificationsTexts.viewService}
          </Link>
        )}
      ...
      <button
        ...
        onClick={handleClose}
        disabled={deleting}
        aria-label={notificationsTexts.closeAlt}

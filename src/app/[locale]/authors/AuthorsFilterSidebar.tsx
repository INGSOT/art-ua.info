...
export default function AuthorsFilterSidebar({
    ...
    const participantOptions: { id: ParticipantFilter; label: string }[] = [
        { id: "artist", label: t("list.participantArtist") },
        { id: "organization", label: t("list.participantOrganization") },
        { id: "team", label: t("list.participantTeam") },
    ];

    return (
        <div className={`flex flex-col w-full lg:w-[300px] flex-shrink-0 ${className}`}>
            <Unit title={t("sidebar.participantsTitle")} isLast={!parameters.length}>
            ...
            {!!parameters.length && (
                <Unit title={t("sidebar.parametersTitle")} isLast>

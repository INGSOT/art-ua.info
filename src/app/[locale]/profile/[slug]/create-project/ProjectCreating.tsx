...
export default function ProjectCreating() {
  ...
  useEffect(() => {
    ...
    projectsAPI
      ...
      .catch(() => {
        if (!cancelled) setEditLoadError(t("publication.messages.editLoadError"));
      })
      .finally(() => {
    ...
    parameterCatalog.forEach((param) => {
      ...
      if (param.type === "list") {
        if (answer?.valueId == null) {
          errors[`param_${param.id}_value`] = [t("specifications.errorValue")];
        }
      } else {
        if (!answer?.value.uk?.trim()) {
          errors[`param_${param.id}_uk`] = [t("specifications.errorUa")];
        }
        if (!answer?.value.en?.trim()) {
          errors[`param_${param.id}_en`] = [t("specifications.errorEn")];
        }
      }
    });
    return errors;
  ...
  const handleNextTab = () => {
    ...
    if (activeTab === "name") {
      const errors: Record<string, string[]> = {};
      if (!projectNameUa.trim()) errors["title.uk"] = [t("name.errorUa")];
      if (!projectNameEn.trim()) errors["title.en"] = [t("name.errorEn")];
      if (!selectedArtField) errors.art_category = [t("name.errorArtField")];
    ...
    if (activeTab === "media" && workGalleryItems.length === 0) {
      setFieldErrors((prev) => ({ ...prev, final_result: [t("media.workError")] }));
      return;
    }
    ...
    if (activeTab === "description") {
      ...
      if (!descriptionUa.trim()) {
        errors["short_description.uk"] = [t("description.errorUa")];
      }
      if (!descriptionEn.trim()) {
        errors["short_description.en"] = [t("description.errorEn")];
      }
      if (Object.keys(errors).length > 0) {
        setFieldErrors((prev) => ({ ...prev, ...errors }));
        return;
  ...
  const handlePublish = async (): Promise<{ type: "success" | "error"; text: string }> => {
    ...
    try {
      ...
      return { type: "error", text: getApiErrorMessage(error, t("publication.messages.publishError")) };
    }
  };
  ...
  const handleSaveDraft = async (): Promise<{ type: "success" | "error"; text: string }> => {
    ...
    try {
      if (editSlug) {
        ...
        setProjectStatusLabel(t("publication.statusDraft"));
        return {
          type: "success",
          text: wasPublished
            ? t("publication.messages.draftUnpublished")
            : t("publication.messages.draftSaved"),
        };
      }
      ...
      return { type: "success", text: t("publication.messages.draftSaved") };
    } catch (error) {
      const errors = getApiFieldErrors(error);
      if (errors) setFieldErrors(errors);
      return { type: "error", text: getApiErrorMessage(error, t("publication.messages.draftError")) };
    }
  };
  ...
  const handleDelete = async (): Promise<{ type: "success" | "error"; text: string }> => {
    if (!editSlug) {
      resetFormState();
      return { type: "success", text: t("publication.messages.formCleared") };
    }

    try {
      await projectsAPI.myDelete(editSlug);
      router.push(withProfileId("/profile/projects", profileSlug));
      return { type: "success", text: t("publication.messages.deleteSuccess") };
    } catch (error) {
      return { type: "error", text: getApiErrorMessage(error, t("publication.messages.deleteError")) };
    }
  };
  ...
  return (
    ...
    >
      ...
      {activeTab === "name" && (
        <NameSection
          projectNameLabel={t("name.label")}
          projectNamePlaceholder={t("name.placeholderUa")}
          projectNamePlaceholderEn={t("name.placeholderEn")}
          artFieldButton={t("name.artFieldButton")}
      ...
      {activeTab === "media" && (
        <MediaSection
          ...
          tagsUa={tagsUa}
          tagsEn={tagsEn}
          addCoverText={t("media.addCoverText")}
          addCoverOptional={t("media.addCoverOptional")}
          workUploadText={t("media.workUploadText")}
          tagsLabel={t("media.tagsLabel")}
          tagsPlaceholder={t("media.tagsPlaceholderUa")}
          tagsPlaceholderEn={t("media.tagsPlaceholderEn")}
          tagsHint={t("media.tagsHint")}
      ...
      {activeTab === "publication" && (
        <>
          <PublicationPreviewSection
            ...
            statusLabel={projectStatusLabel || t("publication.statusDraft")}
          />
          <ProjectPublication
        ...
        <div className="w-full max-w-[1000px] flex justify-center">
          ...
          >
            ...
            <div className="flex items-center justify-center w-[60px] border-l border-[#343434]">
              <Image src="/grey_triangle_right.svg" alt={t("next")} width={24} height={24} />
            </div>
          </button>
        </div>
      )}
      ...
      <AddProjectCover
        ...
        customTitle={t("addImageModal.title")}
        noAnimation={true}
        onBack={() => {
          setIsWorkImageModalOpen(false);
          setIsWorkModalOpen(true);

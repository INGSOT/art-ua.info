...
export default function ListOfServices() {
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            try {
                const result = await publicServicesAPI.browse(language, { performer_slug: slug, per_page: 50 });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-4 lg:gap-8 px-4 md:px-[30px] lg:pl-[75px] lg:pr-[75px]">
                {myServices.map((service) => (
                    <ServiceCard
                        ...
                        overlayButtonLabel={formatOverlayLabel(service, t("services.negotiablePrice"), t("services.priceFromPrefix"))}
                        title={service.title}
                        footer={{
                          variant: "order",
